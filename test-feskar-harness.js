...
function setup(randomFn, extras = {}) {
  ...
  const habits = JSON.parse(fs.readFileSync('./data/feskar_habits.json', 'utf8'));
  const cmd = JSON.parse(fs.readFileSync('./data/feskar_vanguard_command.json', 'utf8'));

  const data = {
    id: 'feskar', name: 'Feskar', rarity: 'Epic', breed: 'Champion',
    stats: { str: 34, inst: 58, int: 58, init: 58 },
    affinity: ['cavalry'], weaknesses: ['siege']
  };
  const stars = extras.stars != null ? extras.stars : 10;
  const fes = new Character(data, 0, 1, { level: 16, stars, habitRank: 1 });
  fes.setTroopType('cavalry');
  loadKit(fes, habits, cmd);
  ...
  if (extras.e0Hp != null && e0) e0.currentHealth = extras.e0Hp;
  if (extras.e1Hp != null && e1) e1.currentHealth = extras.e1Hp;
  if (extras.e2Hp != null && e2) e2.currentHealth = extras.e2Hp;

  const teamA = [left, fes, right].filter(Boolean);
  ...
  return { battle, fes, left, right, e0, e1, e2 };
}

function dumpEngine(label, fes, left, right, e0, e1, e2) {
  const lines = [];
  const snap = (c) => {
    if (!c) return null;
    return {
...
export {
  dummy, loadKit, setup, dumpEngine, check, checks, rN, rFmt
};
