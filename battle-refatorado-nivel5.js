...
class Battle {
  ...
  selectCharacterAction(character) {
    ...
    // Tentar usar habit se disponível e não bloqueada
    if (canUseAbilities(character)) {
      const usableHabits = character.getUsableHabits(this.currentRound, 'each_round');
      
      if (usableHabits.length > 0) {
        // Selecionar primeira habit disponível
        return {
          type: 'habit',
          habit: usableHabits[0]
        };
      }
    }
